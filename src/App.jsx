...
function App() {
  ...
  const testConnection = async () => {
    try {
      ...
      if (data.success) {
        ...
        log.info('API 연결 성공', { ...data, apiUrl });
      } else {
        setWorkStatus(prev => ({ 
          ...prev, 
          isConnected: false, 
        ...
        log.error('API 연결 실패', { ...data, apiUrl });
      }
    } catch (error) {
      setWorkStatus(prev => ({ 
        ...prev, 
      ...
      log.error('연결 테스트 실패', { 
        error: error.message, 
        apiUrl: getApiEndpoint('/api/test-connection') 
      });
    }
  };
  ...
  const startStreaming = async (message, config = {}) => {
    ...
    try {
      const eventSource = new EventSource(getEventSourceUrl(`/api/stream?${params}`));
      ...
      for (let i = 0; i < projectInfo.pages.length; i++) {
        ...
        try {
          ...
          await new Promise((resolve) => {
            ...
            const modificationEventSource = new EventSource(getEventSourceUrl(`/api/stream?${params}`));
